...
import { temperatureToRGB } from './adjustment-math';
...
export async function applyAdjustments(
  ...
  pipeline = await applyLightAdjustments(pipeline, adjustments.light, channels);

  // 2. COLOR ADJUSTMENTS
  pipeline = await applyColorAdjustments(pipeline, adjustments.color, channels);
...
async function applyLightAdjustments(
  ...
  // Exposure: Linear brightness adjustment
