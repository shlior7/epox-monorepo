...
export function ReferenceModal({ value, onSave, onClose }: BubbleModalProps<ReferenceBubbleValue>) {
  const [selectedImages, setSelectedImages] = useState<string[]>(
    value.image ? [value.image.url] : []
  );
  const [selectedItems, setSelectedItems] = useState<
    Array<{ url: string; sourceType: InspirationSourceType }>
  >(value.image ? [{ url: value.image.url, sourceType: value.image.sourceType }] : []);
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="mb-4">
            ...
            <p className="text-sm text-muted-foreground">
              Choose from explore, saved images, or upload your own. Select multiple to add as
              separate references.
            </p>
          </div>
          <InspirationStep
