...
/**
 * Navigation helpers for common user flows in the application
...
export class NavigationHelper {
   ...
   */
  async goToDashboard() {
    await this.page.goto(`/${this.clientId}`);
    await this.page.waitForLoadState('networkidle');
  }

  /**
   * Navigate to product studio for a specific product
   * @param productId - The product ID to navigate to
   */
  async goToProductStudio(productId: string) {
    await this.page.goto(`/${this.clientId}/${productId}`);
    await this.page.waitForLoadState('networkidle');
  }
   ...
   */
  async goToProductSettings(productId: string) {
    await this.page.goto(`/${this.clientId}/${productId}/settings`);
    await this.page.waitForLoadState('networkidle');
  }

  /**
   * Navigate to collection studio (scene studio)
   * @param studioId - The collection/studio ID
   */
  async goToCollectionStudio(studioId: string) {
    await this.page.goto(`/${this.clientId}/scene-studio/${studioId}`);
    await this.page.waitForLoadState('networkidle');
  }
   ...
   */
  async goToClientSettings() {
    await this.page.goto(`/${this.clientId}/settings`);
    await this.page.waitForLoadState('networkidle');
  }
  ...
  async getConfigPanelState(panelSelector: string = '[data-testid="config-panel"]') {
    ...
    const state = await this.page.evaluate((selector) => {
      ...
      const labels = Array.from(panel.querySelectorAll('label, h1, h2, h3, h4, h5, h6')).map((el) => el.textContent?.trim());
  ...
  async getPageStateSummary() {
    return await this.page.evaluate(() => {
      return {
        ...
        errorMessages: Array.from(document.querySelectorAll('[role="alert"], .error, [data-error]')).map((e) => e.textContent?.trim()),
      };
    });
  }
