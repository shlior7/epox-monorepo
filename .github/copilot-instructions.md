...
1. **Set the stage** – start every file with a goal comment. *“// Build an LRU cache with eviction callback”*.\[^1]
...
*Tip: annotate each cycle in commit messages*:

> `git cz` → *feat(cache): 🎯 Cycle 2 – adds eviction policy, 97% cov.*
