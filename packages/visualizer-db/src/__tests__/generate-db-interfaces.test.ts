...
import {
  ...
  toCamelCase,
  toPascalCase,
} from '../../scripts/generate-db-interfaces';
...
describe('generate-db-interfaces', () => {
  ...
  it('resolves enums, arrays, and nullability', () => {
    ...
    expect(
      resolveColumnType(
        { dataType: 'USER-DEFINED', udtName: 'asset_status', isNullable: false },
        enumTypeNames
      )
    ).toBe('AssetStatus');
    ...
    expect(
      resolveColumnType(
        { dataType: 'ARRAY', udtName: '_asset_status', isNullable: true },
        enumTypeNames
      )
    ).toBe('AssetStatus[] | null');

    expect(
      resolveColumnType(
        { dataType: 'ARRAY', udtName: '_uuid', isNullable: false },
        enumTypeNames
      )
    ).toBe('string[]');
    ...
    expect(
      resolveColumnType(
        { dataType: 'numeric', udtName: 'numeric', isNullable: false },
        enumTypeNames
      )
    ).toBe('number');
  });
