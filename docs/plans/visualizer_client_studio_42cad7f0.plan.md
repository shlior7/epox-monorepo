...
overview: Create `visualizer-client` - a magical, simplified AI studio for bulk product visualization. Users select products from a searchable table, AI analyzes product metadata to determine room/product types, then users select inspiration images (upload, Unsplash, or pinned generated images) tailored to the analysis results.
todos:
  ...
  - id: build-product-table
    content: Build product table with search, filter, sort, and multi-select
    status: pending
    dependencies:
      - create-client-app
  - id: implement-product-analysis
    content: Build metadata-only product analysis API (room types, product types)
    status: pending
    dependencies:
      - build-product-table
  - id: create-analysis-preview
    content: UI to display analysis results and config settings
    status: pending
    dependencies:
      - implement-product-analysis
  - id: build-inspiration-picker
    content: Create inspiration picker with Upload, Unsplash, and Library tabs
    status: pending
    dependencies:
      - create-analysis-preview
  - id: integrate-unsplash
    content: Integrate Unsplash API with pre-filtered suggestions
    status: pending
    dependencies:
      - build-inspiration-picker
  - id: implement-scene-merge
    content: Merge multiple inspiration images into unified scene config
    status: pending
    dependencies:
      - build-inspiration-picker
  - id: build-generation-queue
    ...
    status: pending
    dependencies:
      - implement-scene-merge
  - id: create-results-gallery
    content: Gallery view for generated images with download/pin options
    status: pending
    dependencies:
...
1. **`buildSystemImageGenerationPrompt()`** - Same prompt builder from [`lib/services/prompt-builder.ts`](apps/scenergy-visualizer/lib/services/prompt-builder.ts)
